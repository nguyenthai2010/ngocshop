...
/**
 * EDD_Cross_Sells_Export Class
 *
 * @since 1.1
 */
class EDD_Cross_Sells_Export extends EDD_Export {
	/**
	 * Our export type. Used for export-type specific filters/actions
	 * @var string
	 * @since 1.4.4
	 */
	public $export_type = 'cross_sells';
	...
	public function get_data() {
		...
		foreach ( $payments as $payment ) {
			...
			// skip over payments that don't have cross-sells in them
			if ( ! get_post_meta( $payment->ID, '_edd_payment_cross_sell_total', true ) )
				continue;

			$payment_meta 	= edd_get_payment_meta( $payment->ID );
			$user_info 		= edd_get_payment_meta_user_info( $payment->ID );
			$downloads      = edd_get_payment_meta_cart_details( $payment->ID );
			$total          = edd_csau_get_payment_amount( $payment->ID, 'cross_sell' );
