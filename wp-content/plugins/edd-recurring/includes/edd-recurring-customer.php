...
class EDD_Recurring_Customer {
	...
	public function report_column_subscriber( $value, $customer_id ) {
		$user_id = self::get_user_id_from_customer_id( $customer_id );

		$subscriber = false;
		if( ! empty( $user_id ) ) {
			$user = new WP_User( $user_id );
			if( ( ( is_array( $user->roles ) && in_array( 'edd_subscriber', $user->roles ) ) || user_can( $user_id, 'edit_posts' ) ) && self::is_customer_active( $user_id ) )
				$subscriber = true;
		}
	...
	public function report_column_status( $value, $customer_id ) {
		$user_id = self::get_user_id_from_customer_id( $customer_id );
	...
	public function report_column_expiration( $value, $customer_id ) {
		$user_id = self::get_user_id_from_customer_id( $customer_id );
		$expiration = self::get_customer_expiration( $user_id );
	 ...
	 */
	static private function get_user_id_from_customer_id( $customer_id ) {
		$user_id = 0;
		...
		if ( class_exists( 'EDD_Customer' ) ) {
			...
			$args = array( 'id' => $customer_id, 'number' => 1 );
			$customer = EDD()->customers->get_customer;
