...
define( 'EDD_RECURRING_VERSION',       '2.3' );
...
final class EDD_Recurring {
	...
	private function actions() {
		...
		// Adds the [edd_recurring_update] shortcode for updating payment methods
		add_shortcode( 'edd_recurring_update', array( $this, 'update_payment_form' ) );
	...
	public function paypal_gateway_data( $paypal_args = array(), $purchase_data = array() ) {
		...
		foreach( $purchase_data['cart_details'] as $download ) {
			...
			if( isset( $download['item_number']['options'] ) && isset( $options['recurring'] ) ) {
				...
				$paypal_args = apply_filters( 'edd_recurring_paypal_args_download', $paypal_args, $download );

			}

		}

		return apply_filters( 'edd_recurring_paypal_args', $paypal_args );
	...
	public function receipt( $payment, $receipt_args ) {
		...
		if( ! empty( $details ) ) { ?>
		<tr>
			<td><strong><?php _e( 'Recurring Details', 'edd-recurring' ); ?>:</strong></td>
	...
	public function update_payment_form() {
		...
		if ( ! $show_cc_form ) {
			...
			$form_fields['cc_number'] = array(
				'type'                => 'text',
				'options'             => array(
					'name'            => 'edd-recurring-card-number',
					'label'           => __( 'Credit Card Number', 'edd-recurring' ),
					'placeholder'     => __( 'Credit Card Number', 'edd-recurring' ),
					'autocomplete'    => false
				)
			);

			$form_fields['cc_name'] = array(
				'type'              => 'text',
				'options'           => array(
					'name'          => 'edd-recurring-card-name',
					'label'         => __( 'Name on Card', 'edd-recurring' ),
					'placeholder'   => __( 'Name on Card', 'edd-recurring' ),
					'autocomplete'  => true
				)
			);

			$form_fields['cvc_number'] = array(
				'type'                 => 'text',
				'options'              => array(
					'name'             => 'edd-recurring-cvc-number',
					'class'            => 'small-text',
					'label'            => __( 'CVC Number', 'edd-recurring' ),
					'placeholder'      => __( '###', 'edd-recurring' ),
					'autocomplete'     => false
				)
			);

			$form_fields['exp_head']  = array(
				'type'                => 'legend',
				'options'             => array(
					'text'            => __( 'Card Expiration', 'edd-recurring' )
				)
			);

			$form_fields['exp_month'] = array(
				'type'                => 'month_dropdown'
			);

			$form_fields['exp_year']  = array(
				'type'                => 'year_dropdown',
				'number_of_years'     => 10
			);
	...
	private function verify_profile_update_action( $user_id ) {
		...
		do_action( 'edd_recurring_process_profile_card_update', $user_id, $verified );
